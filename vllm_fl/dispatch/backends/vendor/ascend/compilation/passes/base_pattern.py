...
class BasePattern(ABC):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        ...
        try:
            ...
            if "AddRmsNormBias" not in str(e):
                raise
            logger.warning("Skipping %s: AddRmsNormBias is not supported in this environment", pattern_id)
            return
        except AttributeError as e:
            if "unquantized_gemm" not in str(e):
                raise
            logger.warning("Skipping %s: vllm.unquantized_gemm is not available", pattern_id)
            return
