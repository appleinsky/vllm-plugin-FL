...
def _is_310p() -> bool:
    """Return True if the current Ascend device is a 310P.

    In vllm-plugin-FL we do not depend on vllm-ascend internals, so default to
    False.  Vendors can override this helper if they need 310P-specific pass
    disabling.
    """
    try:
...
class GraphFusionPassManager:
    ...
    def configure(self, config: VllmConfig):
        ...
        if self.ascend_compilation_config.get("fuse_norm_quant", True) and not _is_310p():
        ...
        if self.ascend_compilation_config.get("fuse_muls_add", True) and not _is_310p():
